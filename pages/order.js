    ...
    const listOutMarkers = () => {
        if (markers)
            return markers.map((marker) => {
                return (
                    <div key={marker.id} className={`flex flex-col gap-1 w-full h-full mobile:justify-between ${selectedMarker.id === marker.id ? 'border border-white' : ''}`}>
        ...
        return !isLoaded ?
            (
                ...
                <div className='w-full h-screen flex justify-between pt-[1rem]'>
                    ...
                    <div className='flex flex-col w-1/3 gap-2 h-full bg-slate-800 text-white mobile:w-1/2'>
                        <h1 className='subTitle text-2xl font-bold text-center z-20'>Find your local Weight-Plate</h1>
                        <div className='h-1/2 flex flex-col gap-8 mobile:h-full'>
                            {listOutMarkers()}
                        </div>
                    </div>
